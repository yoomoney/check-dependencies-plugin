...
#### Возможные настройки проверки конфликтов

Проверку конфликтов можно отключить, выставив данную настройку в false:
```groovy
checkDependencies {
   enableVersionConflictCheck = true //default
}
```

Также можно исключить из проверки конкретные пакеты:
```groovy
checkDependencies {
   excludedVersionConflictLibraries = ["ru.yandex.money.common:yamoney-xml-utils", "ru.yandex.money.common:yamoney-json-utils"]
}
```

### Вывод новых доступных версий для внешних и внутренних библиотек

Печатает доступные новые версии внутренних и внешних зависимости при условии, что выполнение происходит в фиче бранче.

#### Возможные настройки проверки конфликтов

С помощью этих настроек можно включать и выключать вывод версий для внутренних и внешних библиотек:
```groovy
checkDependencies {
   showInnerDependencies = true //default
   showOuterDependencies = false //default
}
```

Замечу, что вывод версий для внешних библиотек по умолчанию выключен.
